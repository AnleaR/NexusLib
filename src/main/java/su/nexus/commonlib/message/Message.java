...
import su.nexus.commonlib.placeholders.PlaceholderManager;
	...
	public JsonElement getCompleteValue() {
		...
		for (Map.Entry<String, Object> tag : this.tags.entrySet()) {
			...
			try {
				...
				CommonLibPlugin.getInstance().getLogger().log(Level.SEVERE, "Got exception processing tag " + full + " with value " + value + " for message " + this.key, e2);
			}
		}
